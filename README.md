...
قواعد بخشنامه‌ها به صورت ساختاریافته (JSON) مدل‌سازی شده‌اند. برای متن بخشنامه جدید، مدل زبانی
`ag/gemini-3.6-flash-high` یک پیش‌نویس استخراج می‌کند؛ اما تصمیم نهایی به LLM واگذار نمی‌شود.
هر شرط باید یک نقل‌قول دقیق از متن منبع داشته باشد و قبل از ورود به موتور Go نیازمند تأیید کارشناس است.
هر بخشنامه شامل:
...
### استخراج LLM و کنترل کیفیت

```bash
cd agent
python rule_extractor.py circular.txt \
  --product-id P001 \
  --circular-ref BN-1405/001 \
  --output rule-draft.json
```

خروجی با وضعیت `draft` و `requires_human_approval: true` تولید می‌شود. اعتبارسنجی محلی:

- فقط فیلدهای شناخته‌شده پروفایل و عملگرهای مجاز پذیرفته می‌شوند.
- مقدارهای عددی، لیست‌ها و نوع شرط کنترل می‌شوند.
- `source_quote` هر شرط باید واقعاً در متن بخشنامه وجود داشته باشد.
- قواعد نامعتبر یا حدس‌زده‌شده وارد matching engine نمی‌شوند.
- پس از بازبینی کارشناس، قاعده در داده ساختاریافته بخشنامه ثبت و تست‌های سناریو اجرا می‌شوند.

## LLM

پروتکل پیش‌فرض: **Chat Completions** (`USE_RESPONSES_API=false`). مدل استقرار: `ag/gemini-3.6-flash-high`.
